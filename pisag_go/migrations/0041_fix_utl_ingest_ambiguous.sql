...
BEGIN
  ...
  -- fast path: if exists -> duplicate
  SELECT e.id, e.status INTO v_id, v_row_status
  ...
  -- payload existence check: if missing, clear link (status becomes review_required below)
  ...
  VALUES (
    ...
    CASE
      ...
      WHEN p_payload_evidence_asset_id IS NULL AND p_provider_event_id IS NULL AND v_source='webhook' THEN 'review_required'
      ELSE 'ingested'
    END::varchar(16),
    0,
  ...
  utl_event_id := v_id;
  status := v_row_status::varchar(16);
  event_key := v_event_key::varchar(128);
  posting_key := v_posting_key::char(64);
  RETURN NEXT;
END;
