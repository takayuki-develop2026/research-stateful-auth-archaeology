...
func (a *StubPreprocessAdapter) ExecutePreprocess(ctx context.Context, in run.PreprocessExecutionInput) (run.PreprocessExecutionOutput, error) {
	...
	outputHash := stubSHA256Hex(summary)
	...
	return run.PreprocessExecutionOutput{
		...
		SummaryText:     summary,
		ConfidenceScore: stubFloatPtr(0.95),
		...
		Metadata: map[string]any{
			...
			"selection":  stubPreprocessSelectionKinds(in.Selection),
		},
	}, nil
}

func stubPreprocessSelectionKinds(sel run.EngineSelection) []string {
...
func stubSHA256Hex(s string) string {
	sum := sha256.Sum256([]byte(s))
	return hex.EncodeToString(sum[:])
}

func stubFloatPtr(v float64) *float64 {
	return &v
}