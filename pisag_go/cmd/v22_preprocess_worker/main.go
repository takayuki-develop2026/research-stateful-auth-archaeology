...
func main() {
	...
	execUC := &usecase.ExecuteV22PreprocessTaskUseCase{
		Tasks:               taskRepo,
	...
	if err != nil {
		log.Fatalf("execute preprocess task: %v", err)
	}

	log.Printf("v22 preprocess worker completed: task_id=%d status=%s", out.Task.ID, out.Task.Status)
}
