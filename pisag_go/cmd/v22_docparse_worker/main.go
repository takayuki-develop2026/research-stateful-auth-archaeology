...
func main() {
	...
	execUC := &usecase.ExecuteV22DocParseTaskUseCase{
		Tasks:               taskRepo,
		...
		DocParsePort:        &worker.StubDocParseAdapter{},
	}

	out, err := execUC.Handle(ctx, usecase.ExecuteV22DocParseTaskInput{
		ProjectID:       projectID,
		TaskID:          taskID,
		DestinationKind: destinationKind,
	})
	if err != nil {
		log.Fatalf("execute docparse task: %v", err)
	}

	log.Printf("v22 docparse worker completed: task_id=%d status=%s", out.Task.ID, out.Task.Status)
}
