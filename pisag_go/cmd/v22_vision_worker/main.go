...
func main() {
	...
	execUC := &usecase.ExecuteV22VisionTaskUseCase{
		Tasks:               taskRepo,
	...
	out, err := execUC.Handle(ctx, usecase.ExecuteV22VisionTaskInput{
		ProjectID:       projectID,
		TaskID:          taskID,
		DestinationKind: destinationKind,
	})
	if err != nil {
		log.Fatalf("execute vision task: %v", err)
	}

	log.Printf("v22 vision worker completed: task_id=%d status=%s", out.Task.ID, out.Task.Status)
}
