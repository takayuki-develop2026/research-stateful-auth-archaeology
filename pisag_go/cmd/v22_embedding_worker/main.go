...
func main() {
	...
	execUC := &usecase.ExecuteV22EmbeddingTaskUseCase{
		Tasks:               taskRepo,
		...
		EmbeddingPort:       &worker.StubEmbeddingAdapter{},
	}

	out, err := execUC.Handle(ctx, usecase.ExecuteV22EmbeddingTaskInput{
		ProjectID:       projectID,
		TaskID:          taskID,
		DestinationKind: destinationKind,
	})
	if err != nil {
		log.Fatalf("execute embedding task: %v", err)
	}

	log.Printf("v22 embedding worker completed: task_id=%d status=%s", out.Task.ID, out.Task.Status)
}
